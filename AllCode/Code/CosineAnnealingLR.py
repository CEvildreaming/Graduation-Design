...
parser.add_argument("--model_names", type=str, default="efficientnet-b0")
parser.add_argument("--pre_trained", type=bool, default=False)
parser.add_argument("--classes_num", type=int, default=8)
parser.add_argument("--dataset", type=str, default="AllCode/ABCD/Dataset")
...
parser.add_argument("--disable_tsne", type=str, default="False")
...
def generate_evaluation_plots(model, test_loader, save_path):
    ...
    for i, color in zip(range(n_classes), cycle(colors_list)):
        ...
        roc_auc = auc(fpr, tpr)
        plt.plot(fpr, tpr, color=color, lw=2, label='{} (AUC = {:0.2f})'.format(class_names[i], roc_auc))
    ...
    for i, color in zip(range(n_classes), cycle(colors_list)):
        ...
        plt.plot(recall, precision, color=color, lw=2, label='{}'.format(class_names[i]))
    plt.xlabel('Recall')
...
if __name__ == "__main__":
    tb_path = "runs/{}/{}/{}".format(dataset_name, args.model_names, args.exp_postfix)
    ...
    with open(log_file, "a") as f:
        f.write(f"Final Test Loss: {test_loss:.2f} Acc: {test_acc:.2f}\n")
